let menu = [{
	...
	},
	{
		id: 2,
		nama: 'Mix Donuts',
		detail: 'Campuran dari berbagai toping ini sangat lezat.',
		kategori: 'mixdonut',
		harga: 17000,
		foto: 'donat2.png'
	},
	{
		id: 3,
		nama: 'Snow Donuts',
		detail: 'Kelembutan Snow Donuts yang nikmat terasa dimulut.',
		kategori: 'donut',
		harga: 18500,
		foto: 'donat4.png'
	},
	{
		id: 4,
		nama: 'Choco Mix Donuts',
		detail: 'Paduan coklat dan campuran topin yang menjadikan Choco Mix Donut sangat lezat.',
		kategori: 'donut',
		harga: 19000,
		foto: 'donat3.png'
	},
	{
		id: 5,
		nama: 'Choco White Donuts',
		detail: 'Coklat yang nikmat terpadu di dalam Choco White Donuts ini.',
		kategori: 'donut',
		harga: 16000,
		foto: 'donat5.png'
	},
	{
		id: 6,
		nama: 'Mix Choco Donuts',
		detail: 'Campuran toping sangat layak dicoba',
		kategori: 'mixdonut',
		harga: 15000,
		foto: 'donat6.png'
	}
...
function loadMenu() {
	...
	for (i in data_menu) {
		var menu_item = `		
			<div class="card border-warning mb-3 col-11 mx-auto rounded my-2 p-3" onClick="addToCart(` + data_menu[i].id + `)">
		...
		if (data_menu[i].kategori == 'donut') {
			donut += menu_item
		} else if (data_menu[i].kategori == 'pinkdonut') {
			pinkdonut += menu_item
		}
		else if (data_menu[i].kategori == 'mixdonut') {
			mixdonut += menu_item
		}
	}
	$("#donut").html(donut)
	$("#pinkdonut").html(pinkdonut)
	$("#mixdonut").html(mixdonut)
...
function loadCart() {
	...
	if (localStorage.cart) {
		...
		for (i in cart) {
			...
			<img src="img/menu/` + cart[i].foto + `" style="width: 80%" class="card-img img-fluid" alt="card image">
		</div>
		...
		<div class="col-12 pl-0 text-right">
			...
			var diskon = total_cart * 10 / 100
			diskon = parseInt(diskon)
			$("#diskon").html(formatRupiah(diskon))
			var total_bayar = total_cart - diskon
			$("#total-bayar").html(formatRupiah(total_bayar))
		}
	} else {
		$("#cart-info").hide()
		data_cart += `<div class="alert alert-danger">Keranjang anda masih kosong.
		</div>
...
function loadOrder() {
	...
	if (localStorage.order) {
		...
		for (i in order) {
			...
			var diskon = parseInt(total_pesanan * 10 / 100)
			var total_bayar = total_pesanan - diskon
...
function addOrder() {
	...
	if (localStorage.cart) {
		...
		var msg_order = ''
		if (liff.isLoggedIn()) {
			msg_order += user.displayName + ' telah memesan Donuts! '
		} else {
			msg_order += 'Terimakasih telah memesan! Anda memesan '
		}
		...
		var diskon = parseInt(total_pesanan * 10 / 100)
		var total_bayar = total_pesanan - diskon
		msg_order += ' dengan total pesanan Rp ' + formatRupiah(total_bayar) + '. Silahkan ditunggu ya'
...
function showOrder(id) {
	...
	var diskon = parseInt(total_order_detail * 10 / 100)
	$("#diskon-order-detail").html(formatRupiah(diskon))

	var total_bayar = total_order_detail - diskon
...
function liffOpenWindow() {
	if (!liff.isInClient()) {
		...
		liff.openWindow({
			url: 'https://hellodonuts.herokuapp.com/',
			external: true
		})
	}
}
